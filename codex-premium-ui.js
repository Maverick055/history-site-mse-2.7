(function () {
    ...
    function rankTickets(query) {
        ...
        return getTickets()
            .map((item) => {
                ...
                const content = normalize(stripMarkup(item.content));
                ...
                terms.forEach((term) => {
                    ...
                    score += Math.min(haystack.split(term).length - 1, 6);
                });
    ...
    function ensureSearchPanel(input) {
        ...
        if (parent && !parent.classList.contains('codex-search-wrap')) {
            ...
            glow.className = 'codex-search-backdrop';
            parent.insertBefore(glow, input);
        }
    ...
    function renderSearchPanel(query, sourceInput) {
        hidePanels();
        renderCommandPalette(query, sourceInput);
        return;
    ...
    function installSearch(originalHandleSearch) {
        ...
        document.addEventListener('focusin', (event) => {
            const input = event.target.closest(searchInputSelector);
            if (!input) return;
            renderCommandPalette(input.value, input);
        });
        ...
        document.addEventListener('click', (event) => {
            ...
            if (clear) {
                ...
                if (typeof originalHandleSearch === 'function') originalHandleSearch.call(window, '');
                filterTOC('');
                hidePanels();
                hideCommandPalette();
                input?.focus();
